...
all: serial.out serial_gprof.out omp.out
...
serial_gprof.out: src/main_serial.c src/image_io.h src/image_io.c src/segmentation.h src/segmentation_serial.c
	$(CC) $(GPROF) $(CC_OMP) $(CC_FLAGS) -o serial_gprof.out src/main_serial.c src/image_io.c src/segmentation_serial.c -lm
